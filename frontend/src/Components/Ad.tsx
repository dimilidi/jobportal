...
return (
  <div className='w-full cursor-pointer sm:px-[0px] md:py-1 max-w-[550px] xl:max-w-[680px]'>
    <div className='h-[110px] w-full md:w-[97%] flex justify-between items-center
    border-lightBeige border-t-2 text-[14px] transition duration:1000 hover:ease-in-out
    hover:shadow-lg hover:bg-white hover:bg-opacity-50 hover:border-y-2 md:h-[100px] md:border-y-2'>
      ...
      className='w-[150px] md:w-[300px] pl-[20px] md:pl-0 flex flex-col justify-center items-start md:items-center md:flex-row md:justify-between'
      >
      <div className='w-[80px] lg:w-[150px] flex 2xl:w-[300px]'>
      {/* TITLE, SECTOR */}
        <div className='w-[185px] relative text-textBlack sm:full'>
          <p className='truncate text-[14px] sm:text-[18px] text-textBlack'>
          {ad.title}
          </p>
          <p className='z-40 absolute top-0 p-2 opacity-0 transition
            hover:visible bg-background text-darkGray text-[14px] rounded-md sm:text-[18px]'>{ad.title}</p>
          {/* SECTOR */}
      ...
      <div className='relative flex sm:flex-col justify-start gap-1 w-[100px] sm:gap-2'>
      {/* LOCATION */}
      ...
      md:w-[150px] md:flex-row md:self-center md:items-center md:justify-between '>
      {/* CREATED AT */}
        <div className='text-[14px] text-textBlack text-opacity-50 md:ml-[20px] md:text-[16px]'>
        <p>{new Date(ad.createdAt).toLocaleDateString()}</p>
      </div>

      {/* WAGE */}
      <div 
      className='w-[70px] h-[60px] flex justify-center items-center rounded-t-[20px] text-[14px]
      text-textBlack text-opacity-70 bg-lightBeige
      md:mr-[-15px] md:h-[100px] md:w-[50px] md:rounded-r-[20px]
